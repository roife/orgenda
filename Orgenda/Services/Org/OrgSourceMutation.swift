...
struct OrgSourceMutation: Hashable, Sendable {
    ...
    /// Builds a `TODO -> NEXT -> DONE -> TODO` workflow mutation while
    /// retaining the tree-sitter node's trailing spaces and tabs verbatim.
    static func workflowCycle(
        in source: String,
        startByte: Int,
        endByte: Int
    ) throws -> OrgSourceMutation {
    ...
    private static func validatedRange(
        ...
        guard let lowerBound = stringIndex(atUTF8Offset: startByte, in: source) else {
            throw OrgSourceMutationError.invalidStringBoundary(byteOffset: startByte)
        }
        guard let upperBound = stringIndex(atUTF8Offset: endByte, in: source) else {
    ...
    private static func splitTrailingHorizontalWhitespace(
        ...
        while trailingStart > text.startIndex {
