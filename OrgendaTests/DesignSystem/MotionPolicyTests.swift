...
final class MotionPolicyTests: XCTestCase {
    ...
    func testDayKeysUseNumericMonthsInNonGregorianCalendars() throws {
        let date = try XCTUnwrap(ISO8601DateFormatter().date(from: "2023-03-26T12:00:00Z"))
        var chinese = Calendar(identifier: .chinese)
        chinese.timeZone = try XCTUnwrap(TimeZone(secondsFromGMT: 0))
        XCTAssertEqual(date.orgendaDayKey(in: chinese), "0040-02-05")
    }
