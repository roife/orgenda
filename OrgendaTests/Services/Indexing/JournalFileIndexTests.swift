...
final class JournalFileIndexTests: XCTestCase {
    @MainActor
    func testSelectedDayTimelineSortsByTimeAndThenBySourcePosition() throws {
        let source = """
        * 2032-01-05 Mon
        ** 09:15 Morning
        Early note.
        ** 21:40 Evening
        Late note.
        ** 21:40 Latest evening
        Appended in the same minute.
        * 2032-01-06 Tue
        ** 10:00 Other day
        Must not appear on January 5.
        """
        let entries = JournalFileIndex.entries(in: [document(contents: source)])
        let date = try XCTUnwrap(entries.first?.date)
        let store = WorkspaceStore(journalEntries: Array(entries.reversed()))

        XCTAssertEqual(store.journal(on: date).map(\.title), ["Latest evening", "Evening", "Morning"])
        XCTAssertEqual(store.journal(on: date.adding(days: 1)).map(\.title), ["Other day"])
        XCTAssertTrue(store.journal(on: date.adding(days: -1)).isEmpty)
    }
